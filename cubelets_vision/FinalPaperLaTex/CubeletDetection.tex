...
We generated three sets of 10-14 images each. Within each set, the images were taken with the same setup and lighting conditions. Each image takes a picture of an arrangement of Cubelets. The Cubelet arrangements were randomly determined by us.\\[3pt]
...
One image from each set was used as ground truth. For this image, we hand-labeled the location of one Cubelet of each color with (x,y) information. For each set of images, we determined a template radius, corresponding roughly to the circular portion of the cube face.   \\[6pt]
...
% increase table row spacing, adjust to taste
\renewcommand{\arraystretch}{1.3}
\caption{Cube Localization and Pose Estimation}
\label{table_1}
\centering
\subfloat[Average Offset from Actual Center and Standard Deviation]{
\begin{tabular}{|c|c|c|}
\hline
...
\hfil
\centering
\subfloat[Average Error in Rotation and Standard Deviation]{
\begin{tabular}{|c|c|c|}
\hline
 & Pixel Distance from Actual &  Standard Deviation\\
\hline
...
% increase table row spacing, adjust to taste
\renewcommand{\arraystretch}{1.3}
\caption{Detection Failure Occurences}
\label{table_4}
\centering
\begin{tabular}{|c|c|c|}
\hline
